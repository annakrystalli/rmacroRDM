...
Managing such datasets for reproducibility and reusability clearly imposes extra computational and data management effort, with the responsibility lying currently on individual researchers to provide it. They may often lack the time, skill, resources or tools needed to achieve good data management practices resulting in one of the weakest link to data access and reusability. Support to facilitate good research data management at the level of individual researchers and labs seems key.

As noted by Morgan, for trait databases, there really isn't even an agreed structure to which data **should** be specified yet. So standardisation is a key starting point for useful data products, followed by. Challenges to reaching consesus on standards stem from diffulties in the very definition of traits was and are well summarised in **Tom Webb's blog post: [Trait databases: the desirable and the possible](http://www.scilogs.com/mola_mola/trait-databases-the-desirable-and-the-possible/)**:
...
Ultimately: without broad buy-in from the trait community into some form of standardised framework, success of researchers using centralised repositories for research and contributing their data to it, is doubtfull.
...
Standardasation of data products allows exploratory apps to be built around them, eg: 
